...
for T_exec in 交易日历 ∩ [start_date, end_date):
    ...
    2. 复权调整：对持仓中当日 factor 与其 last_factor 不同的股票，调整 volume、cost_price 与 price（见"复权处理"）。
...
price      /= factor_ratio                  // 估值价同步调整，停牌除权日市值连续
```
...
- `price` 同步除以 `factor_ratio`：正常交易日 `end_of_day` 会覆盖为当日收盘价，本调整无影响；**停牌除权日无有效收盘价，price 保持复权调整后的值，保证市值连续**。
...
1. **卖出**：在当前持仓中，当日无 `score`（不在当日信号中）的股票**全部卖出，不占 `drop_n` 名额**；其余有 `score` 的持仓按 `score` 升序取最差的 `drop_n` 只卖出（`method_sell = "bottom"`）。因此单日计划卖出只数可能超过 `drop_n`（无 score 持仓数 + `drop_n`）。
...
3. **资金分配**：买单等权分配——每单金额 =（可用现金 + 计划卖出全部成交的预期回款）/ 计划买入只数 `n_buy`；**预期回款为毛额口径：Σ(卖出委托量 × T_exec 日 `deal_price` 列价格)，不预估滑点与费用**（执行层摩擦对策略不可见，见信息边界），实际回款不足时由撮合阶段的资金约束反解裁剪后续买单。委托价格取 T_exec 日 `deal_price`，委托股数 = 每单金额 / 委托价格（最终以 Exchange 撮合与整手取整为准）。金额在生成决策时一次性确定：第 4 条核减买单时只丢弃多余买单；**若实际发生丢弃，剩余买单按实际可用现金重新等权分配**，避免被截断的买单仍按原 `n_buy` 口径只使用部分资金。
...
5. **边界情况**：
   ...
   - `score` 并列时按内部 `code` 升序保证确定性（`SZ000001` 编码为 1，排在 `SH600000` 之前）：买入排名（降序）同分取代码小者优先，卖出排名（升序）同分取代码小者优先；
   - 某日无信号可用：不调仓，持仓原样持有；
...
1. **目标集合**：排名宇宙 = 当日有 `score` 且（当前已持有 **或** 当日可买入）的股票，按 `score` 降序取前 `top_n` 只（同分按内部 `code` 升序，代码小者优先；`SZ` 代码排在 `SH` 主板代码之前）。未持有且当日不可交易（停牌/涨停/无量）的股票不进宇宙、不占 `top_n` 名额；已持有的股票即使当日停牌/涨停也参与排名（避免"停牌一天就被卖出"）。
