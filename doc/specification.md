...
涨跌停基于当日 `deal_price` 对 `pre_close` 判定，开盘竞价结束即可得，属合法可见；策略**不可见**当日 `close` / `high` / `low` 等盘后行情，也不可见信号 `ret` 列。内置策略委托价格统一取当日 `deal_price`。
...
3. **资金分配**：买单等权分配——每单金额 =（可用现金 + 当日卖出回款）/ 待买入只数；委托价格取 T_exec 日 `deal_price`，委托股数 = 每单金额 / 委托价格（最终以 Exchange 撮合与整手取整为准）。
...
- 停牌持仓照常输出一行：`price` 沿用最近一个有效收盘价，`count_day` 照常 +1（持仓仍在）。
- 不输出 CASH 行：各日 `weight` 之和 ≤ 1，差额即为当日现金占比。

## 指标定义（Report 附录）

记组合逐日总资产为 `V_t`（含现金），逐日收益率 `r_t = V_t / V_{t−1} − 1`，基准逐日收益率 `b_t`，年化交易日数 `N = 252`，回测天数 `n`。
...
| 换手率 `turnover`  | 双边换手 = `（当日买入金额 + 当日卖出金额）/ 前一交易日总资产`（首个有成交的交易日用期初资金） |
| 成本率             | `当日交易费用 / 前一交易日总资产`（= export_data 的 `cost` 列）；无成交日记 0                                 |

补充定义：

- **净值口径**：上表公式默认基于**含成本**收益序列 `r_t`（年化、波动、夏普、回撤同此）；如需不含成本口径，将 `r_t` 替换为 `r'_t` 同式计算。

- **首日口径**：首日 `r_0 = 0` 而 `b_0` 为基准当日实际收益，故 `excess_0 = −b_0`（与参考输出一致），不做特殊处理。
...
- 正确性基准：与仓库内参考输出（`report_data.csv` / `hist_position.csv`）对拍。参考输出参数可从文件反推：初始资金 5,000,000，`top_n = 50`，区间 2022-09-30 ~ 2026-07-29。
  ...
  - 数值容差：`> 待定`（浮点与实现顺序差异，建议逐日总资产相对误差 ≤ 1e-9，逐笔成交明细一致）。
