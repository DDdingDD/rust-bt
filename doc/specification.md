...
> 本文档在 v2 基础上补充了主循环时序、复权、整手、策略细节、指标公式等内容，作为开发依据。
> 文中标注 **`> 待定`** 的条目为尚未拍板的决策点，开发时按紧随其后的默认方案实现。
> 与 v2 冲突处以本文档为准（已确认的修正：默认基准为 **zz1000**；复权按 **factor** 处理）。
...
fn main() -> anyhow::Result<()> {
    ...
    let report = bt_result.gen_report("zz1000")?;
...
- 基准覆盖校验：基准数据必须覆盖回测区间的全部交易日，否则**直接报错**。校验在 `gen_report(name)` 选定基准后进行（加载阶段不校验，因为一个文件含多个基准指数）。
...
账户层面持有：`cash`（可用现金）、`account_value`（总资产快照）、`current_position`（持仓）。
...
for T_exec in 交易日历 ∩ [start_date, end_date):
    ...
    2. 生成决策：strategy.gen_decision(signal, current_position, cash, tradable_info) → Order 列表。
    3. 撮合：Exchange.deal_order 逐单撮合，成交价取 T_exec 日的 deal_price 列；
       卖单先于买单处理，卖出回款（扣除费用后）当日即可用于买入。
    4. 复权调整：对 T_exec 日 factor 发生变化的在持股票，调整 cost_price 与 volume（见"复权处理"）。
    5. 日终估值与记账：
...
- **期初空仓**：start_date 首个交易日使用的是 start_date 前一交易日的信号；若 `pred.csv` 不含该日信号，则首日空仓，回测从有信号可用的次日起逐步建仓。

---

## 复权处理（factor）

`stock_bar.csv` 中价格为不复权原始价，`factor` 为累计复权因子。持仓跨除权除息日（`factor` 发生变化）时，按 qlib 语义调整持仓，保持持仓市值与真实盈亏连续：

```text
factor_ratio = factor_today / factor_last   // 在持股票最近两次交易日的 factor 之比
volume     ×= factor_ratio                  // 送转股导致股数增加
cost_price /= factor_ratio                  // 除权导致成本价降低
```

- 现金分红不单独入账，分红收益通过 `factor` 调整隐含体现（与 qlib 一致，factor 已含分红再投资假设）。
- 撮合成交价始终使用**当日原始价**，不做复权；估值 `price` 也使用当日原始收盘价。
- 新买入股票的 `cost_price` = 实际成交价（含滑点，不含费用）。

> 费用不摊入 `cost_price`。费用直接扣减现金（与 qlib 一致）。
...
- **科创板（SH688xxx）**：按"200 股起、之后按 1 股递增"的申报规则处理——买入数量向下取整到 1 股（不再按 100 股取整），不足 200 股则成交量为 0；卖出无整手限制，余额不足 200 股时须一次性全部卖出（本系统卖出始终为全部持仓，天然满足）。
- 取整在成交量裁剪、资金约束反解之后执行。
...
目标持仓：预测分数最高的 `top_n` 只股票。每个**有信号可用的交易日**调仓一次，规则如下（对齐 qlib 语义）：

1. **卖出**：在当前持仓中，按当日可用信号的 `score` 升序取最差的 `drop_n` 只，全部卖出（`method_sell = "bottom"`）。持仓中当日无信号的股票不参与排名、不卖出。
2. **买入**：在未持有的股票中，按 `score` 降序选取新股票（`method_buy = "top"`）。买入只数规则见下方 `only_tradable` 参数说明。
3. **资金分配**：买单等权分配——每单金额 =（可用现金 + 当日卖出回款）/ 待买入只数。
4. **卖不掉的处理**：因跌停/停牌/成交量裁剪导致卖出失败或部分成交的股票保留在持仓中；买入只数按**实际成交金额对应的回款**重新计算，未卖出部分对应的买入份额不再生成（避免超配）。
5. **边界情况**：
...
| `only_tradable` | bool | `false` | 卖出候选限定当日可交易股票。`false`：买入只数 = 实际卖出成功只数（配合第 4 条）；`true`：卖单跳过不可交易股票（跌停股留到下一调仓日再卖），买入只数 = `top_n − 当前持仓数`，按 `score` 降序补齐（仍受资金/成交量约束） |
...
订单成交价取自行情的哪一列，当前支持 `"open"`、`"close"`、`"vwap"`（对应 `stock_bar.csv` 的 `vwap` 列）。
...
- 当 `trade_val × cost_ratio < min_cost` 时按固定费用反解可买数量；现金连最低费用都不够时，订单成交量置 0。
...
- 判定逻辑：加载行情时用涨跌停价反推当日涨跌幅阈值，预计算布尔列：
  ...
  其中 `change` 使用 `deal_price` 参数对应的价格列计算（如 `deal_price = "open"` 时基于开盘价判定触板）；因此 `limit_buy`/`limit_sell` 不能脱离 `deal_price` 预计算，应在 Exchange 构建时按 deal_price 计算。`limit_threshold / 0.1` 是容差比例：如 `0.0985` 表示达到涨停幅度的 98.5% 即判定触板（提前约 1.5 个**相对涨幅百分点**），防止买入"接近涨停、实际无法成交"的股票。

- `limit_buy = true` 时不可买入，`limit_sell = true` 时不可卖出；停牌（`paused = 1` 或 `close` 缺失）一律不可交易。

- `pre_close` 缺失（如上市首日）时该股当日不做涨跌停判定，`limit_buy`/`limit_sell` 均为 false。
...
- **当日新买入的复核**：同一订单列表中买、卖同一股票属于策略错误，直接报错。
...
> TODO：`benchmark.csv` 中还包含 `CSI000400`，其指数名称与别名待确认后补充进映射表。
...
记组合逐日总资产为 `V_t`（含现金），逐日收益率 `r_t = V_t / V_{t−1} − 1`，基准逐日收益率 `b_t`，年化交易日数 `N = 242`（对齐 qlib 日频惯例），回测天数 `n`。
...
| 指标              | 公式                                                                  |
| --------------- | ------------------------------------------------------------------- |
| 日收益率 `return`   | `r_t = V_t / V_{t−1} − 1`（首日 `r = 0`，或以建仓后首个有持仓日开始计算）                          |
...
| 年化波动率           | `std(r_t) × √N`                                                     |
| 夏普比率            | `mean(r_t) / std(r_t) × √N`（无风险利率取 0）                               |
| 最大回撤            | `max_t(1 − 净值_t / max_{s≤t} 净值_s)`                                  |
| 信息比率            | `mean(excess_t) / std(excess_t) × √N`                               |
| 换手率 `turnover`  | 双边换手 = `(当日买入金额 + 当日卖出金额) / 当日总资产`                                  |
| 成本率             | `当日交易费用合计 / 当日成交金额合计`（比率；注意与 export_data 中的 `cost` 列含义不同，后者是费用金额，单位元）；无成交日记 0 |

补充定义：

- **不含成本口径**：`return_without_cost` 基于费用扣减前的资产快照计算——账户需同时维护两条资产序列：含成本 `V_t`（费用扣现金）与不含成本 `V'_t`（费用不扣），`r'_t = V'_t / V'_{t−1} − 1`，`excess_without_cost = r'_t − b_t`。
- **换手率**：只输出双边口径（见上表 `turnover`）。
- **超额收益口径**：由 `gen_report` 的参数 `excess_method: "arithmetic" | "geometric"` 配置，默认 `"arithmetic"`（算术差 `r − b`，对齐 qlib）；`"geometric"` 为几何差 `(1+r)/(1+b) − 1`。
...
- 单元测试覆盖：涨跌停判定、滑点计算、费用与 min_cost 反解、整手取整、factor 复权调整、停牌估值沿用。
- 端到端：用仓库内 `pred.csv` + `stock_bar.csv`（2022-10 起）跑通完整回测，输出 `hist_position.csv` 与报告。
- 正确性基准：待定
